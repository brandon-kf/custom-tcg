...
from custom_tcg.core.util import to_dict
...
class SelectByChoice(Select):
    ...
    def speculate(self: SelectByChoice) -> bool:
        ...
        # Check exact acceptance criteria met, if so, auto-select.
        if (
            self.auto_n
        ...
        # Check if otherwise options are the same anyway, if so, auto-select.
        elif (
            self.auto_n
            and len(self.accept_n) == 1
            ...
            for option in self.options:
                # TODO: Throws not serializable error.  # noqa: E501, FIX002, TD002, TD003
                if to_dict(obj=option) != to_dict(obj=option_1):
                    auto_select = False
                    break
        else:
            auto_select = False
